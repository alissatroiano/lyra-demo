...
export default function AICopilot({ lesson, onTriggerPaidFlow }: AICopilotProps) {
  ...
  const handleSendChatMessage = async (textToSend?: string) => {
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(data.error || data.details || "Failed to get response from Lyra.");
      }
    ...
    <div className="grid grid-cols-1 xl:grid-cols-12 gap-6" id="ai-copilot-container">
      ...
      <div className="xl:col-span-3 flex flex-row xl:flex-col gap-2.5 bg-surface-0 border border-black/[0.05] rounded-2xl p-4 xl:p-4.5 overflow-x-auto shrink-0">
        <div className="hidden xl:block border-b border-black/[0.05] pb-3 mb-2.5">
          <span className="text-[10px] font-bold text-teal-brand uppercase tracking-wider font-mono">Lyra Copilot Hub</span>
          ...
          <div className="flex-1 flex flex-col justify-between space-y-4" id="chat-subpanel">
            ...
            <div className="flex-1 overflow-y-auto max-h-[350px] space-y-4 pr-1.5 scrollbar-thin">
              ...
              {isChatLoading && (
                <div className="flex gap-3 justify-start items-center">
                  ...
                  <span className="text-[11px] text-secondary font-mono animate-pulse">Lyra is thinking...</span>
                </div>
              )}
              {chatError && (
            ...
            >
              <input
                ...
                placeholder={`Ask Lyra in her role as "${chatRole}"...`}
          ...
          <div className="flex-1 flex flex-col justify-between space-y-6 animate-fade-in" id="voice-subpanel">
            <div className="border-b border-black/[0.05] pb-4 flex items-center gap-2">
              ...
              <div>
                ...
                <p className="text-[10px] text-secondary font-sans mt-1">Talk out loud with Lyra in real-time over our Gemini Live API bridge</p>
              </div>
            </div>
              ...
              <div className="max-w-md text-center space-y-8">
                  ...
                  >
                    {voiceStatus === "connected" ? (
                      ...
                      <>
                        ...
                        <span className="text-[9px] font-mono font-bold uppercase">Call Lyra</span>
                      </>
                    )}
                  </button>
                </div>
                ...
                <div className="space-y-2">
                  <h4 className="text-sm font-bold text-primary font-sans uppercase tracking-wider font-mono">
                    {voiceStatus === "connected" ? "🎙️ Lyra is Listening..." : voiceStatus === "dialing" ? "📡 Connecting to Gemini..." : "📞 Line Available"}
                  </h4>
                  <p className="text-xs text-secondary leading-relaxed font-sans font-normal">
                    {voiceStatus === "connected" 
                      ? "Say anything! Your microphone streams at 16kHz to Lyra, and she replies instantly with her sweet Zephyr voice. Give it 2-3 seconds to load." 
                      : "Need quick scaffolding on-the-fly? Start a phone conversation with Lyra. No typing required."}
                  </p>
                </div>
